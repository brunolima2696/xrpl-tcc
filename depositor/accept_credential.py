...
def main() -> None:
    ...
    if credential and credential.get("accepted") and not using_credential_override:
        print("Credential do depositor já foi aceita:")
    ...
    if credential and credential.get("subject") != depositor_wallet.address:
        raise ValueError("A Credential armazenada não pertence ao depositor configurado.")

    client = JsonRpcClient(json_rpc_url)
    transaction = build_accept_transaction(
        subject_address=depositor_wallet.address,
    ...
    if credential is None:
        credential = {
            "issuer": issuer_address,
            "subject": depositor_wallet.address,
            "credential_type": CREDENTIAL_TYPE_OVERRIDE,
            "credential_type_hex": credential_type_hex,
        }
        state.setdefault("credentials", {})["depositor"] = credential
    credential["accepted"] = True
    credential["accept_tx_hash"] = response.result["hash"]
    save_state(state)

    print("Credential do depositor aceita.")
    print("Tx hash:")
